...
이 레퍼런스는 추론 운영에 필요한 **158개 오퍼레이션 / 22개 그룹**을 담고 있습니다.
원본 스펙의 큐 스케줄링(Kueue) 관리 35개는 이 문서의 범위가 아니라 제외했고, 그중
인프라 모니터링·가속기 자원 조회 6개만 `Infrastructure Monitoring` 으로 되살렸습니다.
배치 추론(Batch) 5개와 워크로드 포트포워딩 3개는 스펙에는 있지만 이 환경에는 아직
배포되지 않아 제외했습니다 — 실서버가 해당 경로에 `404 Cannot GET`을 돌려줍니다.

| 그룹 | 오퍼레이션 | 무엇을 하나 |
|---|---|---|
| Endpoints | 17 | 엔드포인트 생성·조회·수정·삭제, 일시정지·깨우기, Sanity Check |
| Admin Endpoints | 10 | 테넌트 전체 엔드포인트 운영 |
| Workloads | 15 | 워크로드 배포·조회·로그·삭제 |
...
| Infrastructure Monitoring | 6 | 클러스터·가속기 메트릭, 리소스 인벤토리·가용량, 헬스 |
