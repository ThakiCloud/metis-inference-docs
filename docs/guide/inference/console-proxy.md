...
**에러 코드가 뭉개집니다.** 프록시는 잘못된 요청을 서버 오류로 감싸는 경우가 있습니다.
생성 모델에 `/embed` 를 호출하면 엔드포인트 직결은 `404` 를 주는데 프록시는
`500 SERVER_ERROR / Unexpected server error` 를 줍니다. 무엇을 고쳐야 할지 알 수 없는
형태라 디버깅이 길어집니다. 자세한 비교는 [에러 처리](/guide/inference/errors) 에 있습니다.
