...
## 상태 코드가 알려주는 것과 알려주지 않는 것

### 에러 메시지가 엔진 내부 문구일 수 있습니다

`400` 의 메시지는 vLLM 이 그대로 올려 보내는 경우가 있어, 필드 이름 대신 파이썬 예외
문구가 담기기도 합니다. 예를 들어 `messages` 를 빈 배열로 보내면 이렇게 옵니다.

```json
{"error":{"message":"list index out of range","type":"BadRequestError","code":400}}
```

메시지가 무엇을 고쳐야 할지 알려주지 않을 때는 **상태 코드로 판단하세요.** `400` 이면
요청 본문 문제이지 서버 문제가 아닙니다.

### 모델 종류가 맞지 않으면 경로에 따라 코드가 다릅니다

생성 모델에 `/v1/embeddings` 를 호출하는 것처럼 **모델 종류와 경로가 어긋난 요청**은
호출 방식에 따라 다른 코드로 돌아옵니다.

| 호출 방식 | 응답 |
|---|---|
| 엔드포인트 주소로 직접 (`POST /v1/embeddings`) | `404` — vLLM 이 그 모델에 해당 경로를 열지 않습니다 |
| [콘솔 프록시](/guide/inference/console-proxy) (`POST .../endpoints/{id}/embed`) | `500` — 프록시가 이 경우를 구분하지 못하고 서버 오류로 감쌉니다 |

::: warning 여기서의 500 은 장애가 아닙니다
콘솔 프록시로 임베딩을 요청했는데 `500 SERVER_ERROR` 가 왔다면, 플랫폼이 아니라
**요청과 모델이 안 맞는 것**일 가능성이 높습니다. 엔드포인트의 `/v1/models` 로 어떤
모델인지 먼저 확인하세요. 임베딩은 임베딩 모델을 띄운 엔드포인트에서만 동작합니다.
:::
