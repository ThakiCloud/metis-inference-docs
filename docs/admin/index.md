...
![Admin AI Inference 대시보드 — 테넌트 전체 용량과 요약](/images/admin/admin-overview.png)

| 번호 | 영역 | 설명 |
|---|---|---|
| 1 | 사이드바 | 일반 앱과 달리 Monitoring(Nodes · Usage Trend · Connected Clusters · Dependencies)과 Admin Managements(Projects)가 더 있습니다. |
| 2 | Capacity | 테넌트 전체 기준의 클러스터 쿼터와 요청량. |
| 3 | Workload Summary | 테넌트 전체 워크로드 상태별 개수. |

여기서 하는 일은 크게 세 가지로 나뉩니다. 첫째는 **엔드포인트 운영**입니다. 테넌트 전체
엔드포인트를 조회하고, 수정·일시정지·재개·삭제하고, Sanity Check 로 살아 있는지 확인하고,
최근 활동 이력을 봅니다. 둘째는 **모니터링**입니다. 노드의 자원 현황, 사용량 추세, 연결된
클러스터, 소프트웨어 의존성(SBOM)을 확인합니다. 셋째는 **카탈로그와 시스템 관리**입니다.
vLLM 실행 오버라이드 카탈로그, 프로젝트, 사용자 스토리지 쿼터, 시스템 설정을 다룹니다.
